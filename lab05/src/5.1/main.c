...
pthread_t producers[MAX_PRODS];
pthread_t consumers[MAX_PRODS];
...
void* producer_task(void* arg) {
    ...
    while (1) {
        ...
        printf("[Producer %lu] Added |type:%u hash:%u size:%u| Total:%d\n",
               pthread_self(), msg->type, msg->hash, msg->size, queue->added_count);
        sleep(1 + rand() % 2);
    }
    return NULL;
}
...
void* consumer_task(void* arg) {
    ...
    while (1) {
        ...
        printf("[Consumer %lu] Removed |type:%u hash:%u size:%u| Total:%d\n",
               pthread_self(), msg->type, msg->hash, msg->size, queue->removed_count);
...
void print_status() {
    ...
    pthread_mutex_unlock(&queue->mutex);
    sem_getvalue(&queue->sem_empty, &free_slots);
